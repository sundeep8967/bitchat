...
class BluetoothMeshService(private val context: Context) {
    ...
    /**
     * Get list of active peers with their details
     */
    fun getPeers(): List<PeerInfo> {
        val activeIds = peerManager.getActivePeerIDs()
        return activeIds.mapNotNull { id -> peerManager.getPeerInfo(id) }
    }

    /**
     * Get RSSI for a specific peer
     */
    fun getPeerRSSI(peerID: String): Int {
        return peerManager.getAllPeerRSSI()[peerID] ?: 0
    }
